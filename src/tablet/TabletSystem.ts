...
import { POINTS_HAZARD_FOUND, scoreAssessment } from "@/tablet/risk";
...
export class TabletSystem {
  ...
  async startSession(sceneId: string, hazards: HazardSpec[]): Promise<void> {
    this.sceneId = sceneId;
    this.hazards = hazards;
    try {
      this.profile = await this.save.ensureProfile();
      this.entries = await this.save.listHazardEntries(this.profile.id, sceneId);
    } catch (err) {
